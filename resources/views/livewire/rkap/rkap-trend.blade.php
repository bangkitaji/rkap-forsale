...
">
  ...
  <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center py-3 mb-4 gap-3">
    <div>
      ...
      <p class="text-muted mb-0 small">
        {{ __('Daftar kegiatan usulan, justifikasi deviasi anggaran, dan rekapitulasi progres pengisian justifikasi.') }}
      </p>
    </div>
    <div class="d-flex gap-2 align-items-center flex-wrap">
      ...
      <button type="button" wire:click="exportExcel"
        class="btn btn-success btn-sm d-flex align-items-center gap-1 shadow-sm text-white" wire:loading.attr="disabled">
        <span wire:loading.remove wire:target="exportExcel"><i class="bx bx-download me-1"></i>
          {{ $activeTab === 'summary' ? __('Export Excel Ringkasan') : __('Export Excel') }}</span>
  ...
  <div class="row">
    <div class="col-12">
      <ul class="nav nav-pills flex-column flex-md-row mb-4">
        <li class="nav-item">
          <button type="button" class="nav-link @if($activeTab === 'trend') active @endif"
            wire:click="$set('activeTab', 'trend')">
            <i class="bx bx-trending-up me-1"></i> {{ __('Trend & Justifikasi') }}
          </button>
        </li>
        <li class="nav-item">
          <button type="button" class="nav-link @if($activeTab === 'summary') active @endif"
            wire:click="$set('activeTab', 'summary')">
            <i class="bx bx-bar-chart-alt-2 me-1"></i> {{ __('Ringkasan Trend') }}
          </button>
        </li>
      </ul>
    </div>
                ...
                <tr>
                  <td colspan="7" class="text-center py-5">
                    <div class="d-flex flex-column align-items-center justify-content-center">
                      ...
                      <h6 class="fw-bold mb-1">{{ __('Tidak ada kegiatan ditemukan') }}</h6>
                      <p class="text-muted small mb-0" style="max-width: 400px;">
                        @if ($search || $filterStatus !== 'all')
                          {{ __('Coba sesuaikan filter atau kata kunci pencarian Anda.') }}
                        @else
                          {{ __('Belum ada data kegiatan RKAP untuk periode terpilih pada biro/organisasi ini.') }}
                        @endif
                      </p>
                    </div>
                  </td>
  ...
  @if ($activeTab === 'summary')
    ...
    <div class="card border-0 shadow-sm mb-4">
      <div class="card-body p-3">
        <div class="row g-3 align-items-end">
          ...
          <div class="col-md-3">
            <label class="form-label small fw-semibold text-muted mb-1">{{ __('Status Pengisian') }}</label>
            <div class="position-relative" x-data="{
                open: false,
                selected: @entangle('filterSummaryStatus').live,
                options: [
                    { value: 'Selesai',      label: 'Selesai' },
                    { value: 'Sedang Diisi', label: 'Sedang Diisi' },
              ...
              <button type="button"
                class="form-select form-select-sm text-start d-flex align-items-center justify-content-between"
                @click="open = !open">
              ...
              <div class="dropdown-menu w-100 p-2 shadow-sm border mt-1"
                ...
                <div class="list-group list-group-flush">
                  <button type="button"
                    ...
                    @click="selected = []">
                    <i class="bx bx-x me-1"></i> {{ __('Hapus Semua Filter') }}
                  </button>
        ...
        <div class="card border-0 shadow-sm h-100">
          <div class="card-body p-3">
            <div class="d-flex align-items-center gap-3">
              ...
              <div>
                <div class="fw-bold fs-4">{{ $summaryData['stats']['total_bureaus'] }}</div>
                <div class="text-muted small">{{ __('Total Biro Terdaftar') }}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="col-6 col-md-3">
        <div class="card border-0 shadow-sm h-100">
          <div class="card-body p-3">
            <div class="d-flex align-items-center gap-3">
              <div class="avatar avatar-md flex-shrink-0">
                <span class="avatar-initial rounded bg-label-primary"><i class="bx bx-list-ol fs-4"></i></span>
              </div>
              <div>
                <div class="fw-bold fs-4">{{ number_format($summaryData['stats']['total_activities']) }}</div>
                <div class="text-muted small">{{ __('Total Kegiatan RKAP') }}</div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <!-- Card 3: Sudah Terjustifikasi -->
      <div class="col-6 col-md-3">
        <div class="card border-0 shadow-sm h-100">
          <div class="card-body p-3">
            <div class="d-flex align-items-center gap-3">
              ...
              <div class="w-100">
                <div class="d-flex justify-content-between align-items-baseline">
                  <span class="fw-bold fs-4 text-success">{{ number_format($summaryData['stats']['filled_activities']) }}</span>
                  <span class="text-muted small">/ {{ number_format($summaryData['stats']['total_activities']) }}</span>
                </div>
                <div class="text-muted small">{{ __('Sudah Terjustifikasi') }}</div>
                <div class="progress mt-1" style="height: 4px;">
                  <div class="progress-bar bg-success" role="progressbar" style="width: {{ $summaryData['stats']['percentage'] }}%"
                    aria-valuenow="{{ $summaryData['stats']['percentage'] }}" aria-valuemin="0" aria-valuemax="100"></div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="col-6 col-md-3">
        <div class="card border-0 shadow-sm h-100">
          <div class="card-body p-3">
            <div class="d-flex align-items-center gap-3">
              <div class="avatar avatar-md flex-shrink-0">
                <span class="avatar-initial rounded bg-label-warning"><i class="bx bx-pie-chart-alt fs-4"></i></span>
              </div>
              <div>
                <div class="fw-bold fs-4 text-warning">{{ $summaryData['stats']['percentage'] }}%</div>
                <div class="text-muted small">{{ __('Persentase Pengisian') }}</div>
              </div>
            </div>
          </div>
        </div>
    ...
    {{-- Progress Summary Table Card --}}
    <div class="card border-0 shadow-sm">
      <div class="card-header border-bottom py-3 d-flex justify-content-between align-items-center">
        <div>
          <h5 class="card-title mb-0 fw-bold">{{ __('Progress Pengisian Justifikasi Trend per Biro') }}</h5>
          <small class="text-muted">{{ __('Daftar unit kerja dan rekapitulasi progres pengisian justifikasi trend.') }}</small>
        </div>
      </div>
      <div class="table-responsive">
        <table class="table table-hover align-middle mb-0" style="font-size: 0.85rem;">
          <thead class="table-light">
            <tr>
              ...
              <th class="text-center">{{ __('Jumlah Kegiatan') }}</th>
              <th class="text-center">{{ __('Terjustifikasi') }}</th>
              <th class="text-center">{{ __('Belum Terjustifikasi') }}</th>
              <th style="width: 170px;">{{ __('Progres') }}</th>
          ...
          <tbody>
            @forelse($summaryData['rows'] as $idx => $row)
              <tr wire:key="summary-bureau-{{ $row['bureau_id'] }}">
                <td>{{ $idx + 1 }}</td>
                <td>
                ...
                <td class="text-center fw-semibold">{{ number_format($row['total_activities']) }}</td>
                <td class="text-center text-success fw-semibold">{{ number_format($row['filled_activities']) }}</td>
                <td class="text-center text-muted">{{ number_format($row['unfilled_activities']) }}</td>
                <td>
                  <div class="d-flex align-items-center gap-2">
                    <div class="progress w-100" style="height: 6px;">
                      <div class="progress-bar bg-primary" role="progressbar" style="width: {{ $row['percentage'] }}%"
                        aria-valuenow="{{ $row['percentage'] }}" aria-valuemin="0" aria-valuemax="100"></div>
                    </div>
                ...
                <td class="text-center">
                  <button type="button" class="btn btn-outline-primary btn-sm d-flex align-items-center gap-1 mx-auto"
                    wire:click="viewBureauDetail({{ $row['bureau_id'] }})"
                    title="{{ __('Lihat rincian kegiatan dan justifikasi biro ini') }}">
                    <i class="bx bx-show"></i> {{ __('Detail') }}
                  </button>
                </td>
              </tr>
            @empty
              <tr>
                <td colspan="9" class="text-center py-5 text-muted">
                  <div class="d-flex flex-column align-items-center justify-content-center">
                    ...
                    <h6 class="fw-bold mb-1">{{ __('Tidak ada data biro ditemukan') }}</h6>
                    <p class="small mb-0">{{ __('Coba sesuaikan filter atau belum ada biro terdaftar pada lingkup ini.') }}</p>
                  </div>
                </td>
              </tr>
            @endforelse
          </tbody>
          @if (count($summaryData['rows']) > 0)
            <tfoot class="table-light fw-bold">
              <tr>
                <td colspan="3">{{ __('TOTAL') }} ({{ $summaryData['stats']['total_bureaus'] }} {{ __('Biro') }})</td>
                <td class="text-center">{{ number_format($summaryData['stats']['total_activities']) }}</td>
                <td class="text-center text-success">{{ number_format($summaryData['stats']['filled_activities']) }}</td>
                <td class="text-center text-muted">{{ number_format($summaryData['stats']['unfilled_activities']) }}</td>
                <td>
                  <div class="d-flex align-items-center gap-2">
                    <div class="progress w-100" style="height: 6px;">
                      <div class="progress-bar bg-success" role="progressbar" style="width: {{ $summaryData['stats']['percentage'] }}%"
                    ...
                    <span class="small fw-semibold text-success">{{ $summaryData['stats']['percentage'] }}%</span>
                  </div>
                </td>
                <td class="text-center">
                  <span class="badge bg-label-info">{{ $summaryData['stats']['filled_activities'] }}/{{ $summaryData['stats']['total_activities'] }} ({{ $summaryData['stats']['percentage'] }}%)</span>
                </td>
                <td></td>
              </tr>
            </tfoot>
