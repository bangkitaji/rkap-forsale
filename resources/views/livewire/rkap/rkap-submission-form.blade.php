                ...
                <div class="d-flex align-items-center gap-3 flex-shrink-0 ms-auto">
                    ...
                    foreach (($wp['activities'] ?? []) as $act) {
                        foreach (($act['budget_items'] ?? []) as $bi) {
                            $wpSubtotal += ($bi['quantity'] ?? 0) * ($bi['unit_price'] ?? 0);
                        }
                    }
                    @endphp
                    <span class="text-muted small">
                        Subtotal Program: <strong class="text-dark fs-6">Rp {{ number_format($wpSubtotal, 0, ',', '.') }}</strong>
                    </span>
                    @if(count($workPlans) > 1)
                    <button type="button" wire:click="removeWorkPlan({{ $wpIdx }})" class="btn btn-sm btn-outline-danger" title="Hapus program kerja">
        ...
        <div class="card-body bg-light-gray p-3">
            ...
            <div class="activity-card p-3 mb-3" wire:key="wp-{{ $wpIdx }}-act-card-{{ $actIdx }}">
                <div class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-3">
                    ...
                    <button type="button" wire:click="removeActivity({{ $wpIdx }}, {{ $actIdx }})" class="btn btn-xs btn-outline-danger" title="Hapus Kegiatan">
                ...
                <div class="row g-3 mb-3">
                    ...
                    <div class="col-md-12">
                        ...
                        <div
                            ...
                            {{-- Hidden select --}}
                            <select
                                wire:model.live="workPlans.{{ $wpIdx }}.activities.{{ $actIdx }}.activity_id"
                                class="d-none"
                                id="act-select-{{ $wpIdx }}-{{ $actIdx }}">
                                <option value=""></option>
                ...
                <div class="row g-3 mb-3">
                    <div class="col-md-12">
                        ...
                        <textarea class="form-control form-control-sm" wire:model.live="workPlans.{{ $wpIdx }}.activities.{{ $actIdx }}.description" rows="2" placeholder="Deskripsi kegiatan..."></textarea>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label small fw-semibold">Target Output</label>
                        <input type="text" class="form-control form-control-sm" wire:model.live="workPlans.{{ $wpIdx }}.activities.{{ $actIdx }}.output_target" placeholder="Misal: 1 sistem, 100 user">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label small fw-semibold">Satuan</label>
                        <input type="text" class="form-control form-control-sm" wire:model.live="workPlans.{{ $wpIdx }}.activities.{{ $actIdx }}.unit" placeholder="Paket, Unit, ...">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label small fw-semibold">Volume</label>
                        <input type="number" class="form-control form-control-sm" wire:model.live="workPlans.{{ $wpIdx }}.activities.{{ $actIdx }}.quantity" min="1">
                    </div>
                </div>
                ...
                <div
                    ...
                    <div class="table-responsive" :style="dropdownOpen ? 'overflow: visible;' : ''">
                        <table class="table table-sm table-bordered align-middle mb-2">
                            ...
                            <tbody>
                                ...
                                <tr wire:key="wp-{{ $wpIdx }}-act-{{ $actIdx }}-group-{{ $gIdx }}-coa" class="{{ $gIdx % 2 == 1 ? 'bg-group-alt' : '' }}">
                                    <td colspan="6"
                                        ...
                                        <div class="position-relative">
                                            ...
                                            @error('workPlans.'.$wpIdx.'.activities.'.$actIdx.'.budget_items.'.$firstIdx.'.coa_id')
                                            <div class="invalid-feedback d-block">{{ $message }}</div>
                                            @enderror
                                            <select
                                ...
                                @endphp
                                <tr wire:key="wp-{{ $wpIdx }}-act-{{ $actIdx }}-bi-{{ $biIdx }}-detail" class="{{ $gIdx % 2 == 1 ? 'bg-group-alt' : '' }}">
                                    ...
                                    <td class="border-top-0">
                                        <input type="number"
                                            class="form-control form-control-sm @error('workPlans.'.$wpIdx.'.activities.'.$actIdx.'.budget_items.'.$biIdx.'.quantity') is-invalid @enderror"
                    ...
                    @endphp
                    <div
                        wire:key="modal-{{ $wpIdx }}-{{ $actIdx }}-{{ $biIdx }}"
                        ...
                        <div
                            ...
                            <div class="px-4 py-3" style="overflow-y: auto; max-height: calc(100vh - 12rem);">
                                ...
                                <div class="mb-4">
                                    ...
                                    @error('workPlans.'.$wpIdx.'.activities.'.$actIdx.'.budget_items.'.$biIdx.'.monthly')
                    ...
                    <button type="button" wire:click="addBudgetItem({{ $wpIdx }}, {{ $actIdx }})" class="btn btn-sm btn-label-secondary mt-2">
            ...
            <div class="text-center my-2">
                <button type="button" wire:click="addActivity({{ $wpIdx }})" class="btn btn-sm btn-outline-primary">
