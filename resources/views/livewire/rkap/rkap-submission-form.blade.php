<div x-data="{
  ...
  <div class="toast-container position-fixed top-0 end-0 p-3 rkap-z-1090">
    <div x-show="showToast" x-transition:enter="transition ease-out duration-300"
      ...
      role="alert" aria-live="assertive" aria-atomic="true" x-cloak>
  ...
  @if ($errors->any())
    <div class="toast-container position-fixed top-0 end-0 p-3 rkap-z-1090"
      wire:key="error-toast-container-{{ microtime(true) }}">
      <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 7000)"
        x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-2"
        x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-200"
        x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 translate-y-2"
        class="bs-toast toast show bg-danger text-white" role="alert" aria-live="assertive" aria-atomic="true"
        x-cloak>
  ...
  @if ($this->isSubmissionClosed)
    <div class="alert alert-warning border-warning d-flex align-items-center gap-3 mb-4 shadow-sm" role="alert">
      ...
      <div>
        ...
        <span
          class="small text-muted">{{ __('Periode pengisian usulan RKAP saat ini sedang ditutup. Anda hanya dapat melihat data dan tidak dapat membuat, mengedit, atau menyimpan usulan.') }}</span>
      </div>
    </div>
  @endif
    ...
    <div class="alert alert-danger border-danger mb-4 p-0 overflow-hidden" role="alert">
      ...
      <div class="px-4 py-3">
        @if ($revisionReason)
          <div class="mb-3">
            <div class="fw-semibold text-danger mb-1"><i
                class="bx bx-comment-error me-1"></i>{{ __('Alasan Pengembalian:') }}</div>
        ...
        <div class="d-flex flex-wrap gap-3">
          @if ($rejectedCount > 0)
            <div
              class="d-flex align-items-center gap-2 bg-danger bg-opacity-10 rounded px-3 py-2 border-start border-warning border-3">
          ...
          @if ($approvedCount > 0)
            <div
              class="d-flex align-items-center gap-2 bg-success bg-opacity-10 rounded px-3 py-2 border-start border-success border-3">
              <i class="bx bx-check-circle text-success"></i>
              <span class="small"><strong class="text-success">{{ $approvedCount }} kegiatan</strong> sudah disetujui
                dan
  ...
  @foreach ($workPlans as $wpIdx => $wp)
    @php
      ...
      $hasApproved = collect($wp['activities'])->contains(
          fn($a) => ($a['approval_status'] ?? 'pending') === 'approved',
      );
    @endphp

    <div class="card mb-4 border-start border-primary border-3"
      wire:key="wp-card-{{ $wpIdx }}-{{ $wp['_uid'] }}">
            ...
            <div x-data="{
              ...
              <div class="input-group">
                ...
                @if ($wp['work_plan_id'] && !$hasApproved)
                  <button type="button" class="btn btn-outline-secondary"
                    ...
                    @click="search = ''; currentLabel = ''; selectedId = null; open = false;"
              ...
              <select wire:model.live="workPlans.{{ $wpIdx }}.work_plan_id" class="d-none"
                ...
                @if ($wp['work_plan_id'])
                  ...
                  @if ($selectedWpForSelect)
                    <option value="{{ $selectedWpForSelect->id }}" selected>{{ $selectedWpForSelect->code }} —
                      {{ $selectedWpForSelect->title }}</option>
                  @endif
                @endif
              </select>
              ...
              <div x-show="open" x-cloak wire:ignore.self
                ...
                <template x-for="wpo in filteredWorkPlans" :key="wpo.id">
                  <div class="px-3 py-2 cursor-pointer dropdown-item small"
                    :class="selectedId == wpo.id ? 'bg-primary text-white' : ''" @click="selectWorkPlan(wpo)">
                    <span class="fw-semibold text-primary" :class="selectedId == wpo.id ? 'text-white' : ''"
                      x-text="wpo.code"></span>
                ...
                <div class="px-3 py-2 text-muted small border-top bg-light"><i
                    class="bx bx-info-circle me-1"></i>{{ __('Ketik untuk menyaring program kerja.') }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="flex-shrink-0 ms-auto d-flex flex-column justify-content-end align-items-end">
            ...
            <span class="text-muted small fw-bold text-primary fs-6 has-tooltip">
              ...
              <span class="custom-tooltip-content tooltip-align-right">
                ...
                @endphp
                @if ($prevProgramData)
                  <div class="fw-semibold text-center border-bottom pb-1 mb-2 text-white">
                    {{ __('RKAP Periode Sebelumnya') }}
                    ({{ $prevPeriod }})</div>
      ...
      <div class="card-body bg-light-gray p-3">
        @foreach ($wp['activities'] as $actIdx => $act)
          @php
            ...
            $hasMappedCoas =
                !$act['activity_id'] || $isPastPeriod || ($selectedActivity && $selectedActivity->coas->isNotEmpty());
          @endphp

          <div class="activity-card p-3 mb-3"
            wire:key="wp-{{ $wpIdx }}-{{ $wp['_uid'] }}-act-card-{{ $actIdx }}-{{ $actUid }}">
            <div class="d-flex justify-content-between align-items-center border-bottom pb-2 mb-3">
              <div class="d-flex align-items-center gap-2">
                ...
                @if ($isApprovedRaw)
                  <span class="badge bg-label-success ms-2"><i
                      class="bx bx-check-circle me-1"></i>{{ __('Disetujui') }}</span>
                @elseif ($isRejected)
                  <span class="badge bg-label-danger ms-2"><i
                      class="bx bx-x-circle me-1"></i>{{ __('Revisi') }}</span>
                @endif
                @if ($isTransferLocked)
                  <span class="badge bg-label-warning ms-2"><i
                      class="bx bx-transfer me-1"></i>{{ __('Proses Transfer') }}</span>
                @endif
              </div>
              ...
              <div class="col-md-6">
                ...
                <div x-data="{
                  ...
                  <div class="input-group">
                    <input type="text"
                      ...
                      placeholder="{{ __('Cari kegiatan...') }}" x-model="search" @focus="open = true"
                      @input="open = true" autocomplete="off"
                      id="act-search-{{ $wpIdx }}-{{ $actIdx }}" @disabled($isApproved)>
                  ...
                  <select wire:model.live="workPlans.{{ $wpIdx }}.activities.{{ $actIdx }}.activity_id"
                    ...
                    @if ($act['activity_id'])
                      @if ($selectedActivity)
                        <option value="{{ $selectedActivity->id }}" selected>{{ $selectedActivity->code }} —
                          {{ $selectedActivity->title }}</option>
                      @endif
                    @endif
                  </select>
                  ...
                  <div x-show="open" x-cloak wire:ignore.self
                    ...
                    <template x-for="a in filteredActivities" :key="a.id">
                      <div class="px-3 py-2 cursor-pointer dropdown-item small"
                        :class="selectedId == a.id ? 'bg-primary text-white' : ''" @click="selectActivity(a)">
                        <div class="d-flex flex-column gap-1">
                          <span class="fw-semibold text-primary" :class="selectedId == a.id ? 'text-white' : ''"
                            x-text="a.code"></span>
                          <span class="text-secondary rkap-font-085" :class="selectedId == a.id ? 'text-white-50' : ''"
                            x-text="a.title"></span>
                        </div>
                      </div>
                    </template>
                ...
                <div class="mt-2 d-flex align-items-center gap-2">
                  <div class="form-check mb-0">
                    <input class="form-check-input" type="checkbox"
                      ...
                      id="ppp-{{ $wpIdx }}-{{ $actIdx }}" @disabled($isApproved)>
                    <label class="form-check-label small fw-semibold text-warning"
                      for="ppp-{{ $wpIdx }}-{{ $actIdx }}">
                ...
                @if ($act['is_past_period_payment'] ?? false)
                  <div class="mt-2" wire:key="ppp-section-{{ $wp['_uid'] }}-{{ $actUid }}">
                    <select
                      class="form-select form-select-sm @error('workPlans.' . $wpIdx . '.activities.' . $actIdx . '.past_period_id') is-invalid @enderror"
                      ...
                      @foreach ($this->pastPeriods as $pp)
                        <option value="{{ $pp->id }}">RKAP {{ $pp->year }} — {{ $pp->title }}
                        </option>
                      @endforeach
                    </select>
                    ...
                    <div class="alert alert-warning d-flex align-items-center mt-2 py-2 px-3 rkap-alert-warning">
                      ...
                      <span>
                        ...
                        @if ($pastPeriodTitle)
                          untuk <strong>{{ $pastPeriodTitle }}</strong>
                        @endif.
                  ...
                  @if (!empty($act['uploaded_files']))
                    <div class="d-flex flex-column gap-1 mt-2">
                      @foreach ($act['uploaded_files'] as $fileIdx => $file)
                        @php
                          $isViewable = in_array(strtolower($file['file_type'] ?? ''), [
                              'pdf',
                              'jpg',
                              'jpeg',
                              'png',
                        ...
                        @endphp
                        <div
                          class="d-flex align-items-center justify-content-between bg-light rounded px-2 py-1 rkap-file-list-item"
                          wire:key="file-item-{{ $wp['_uid'] }}-{{ $actUid }}-{{ $fileIdx }}">
                          <div class="d-flex align-items-center gap-1 text-truncate rkap-file-name">
                            ...
                            @if (isset($file['id']))
                              @if (!$fileExists)
                                <span class="text-truncate text-danger"
                                  title="{{ __('File tidak ditemukan di server') }}">
                                  <i class="bx bx-error-circle text-danger me-1"></i>{{ $file['original_name'] }}
                                  <span class="badge bg-label-danger ms-1"
                                    style="font-size: 0.65rem;">{{ __('Mohon upload ulang') }}</span>
                                </span>
                              @else
                                @if ($isViewable)
                                  <a href="javascript:void(0)" class="text-truncate text-primary"
                                    data-bs-toggle="modal" data-bs-target="#viewFileModalForm-{{ $file['id'] }}">
                                    {{ $file['original_name'] }}
                                  </a>
                                @else
                                  <a href="{{ route('rkap-files.download', $file['id']) }}"
                            ...
                            @else
                              <span class="text-truncate text-muted"
                                title="{{ __('Belum disimpan') }}">{{ $file['original_name'] }} (baru)</span>
                            @endif
                            <span
                              class="text-muted flex-shrink-0 small">({{ number_format(($file['file_size'] ?? 0) / 1024, 1) }}
                              KB)</span>
                          </div>
                          ...
                          <div class="d-flex align-items-center gap-1 flex-shrink-0">
                            @if (!$isApproved)
                              <button type="button"
                                class="btn btn-link text-danger p-0 m-0 border-0 rkap-no-text-decoration"
                        ...
                        @if (isset($file['id']) && $isViewable)
                          <div class="modal fade" id="viewFileModalForm-{{ $file['id'] }}" tabindex="-1"
                            aria-hidden="true" wire:key="view-file-modal-form-{{ $file['id'] }}">
                            <div class="modal-dialog modal-dialog-centered modal-xl">
                              <div class="modal-content text-start">
                                <div class="modal-header">
                                  <h5 class="modal-title"><i
                                      class="bx bx-file me-2 text-primary"></i>{{ $file['original_name'] }}</h5>
                                  <button type="button" class="btn-close" data-bs-dismiss="modal"
                                    aria-label="Close"></button>
                                </div>
                                <div class="modal-body p-0 text-center bg-light">
                                  @if (strtolower($file['file_type']) === 'pdf')
                                    <iframe src="{{ route('rkap-files.view', $file['id']) }}"
                                      class="rkap-preview-frame"></iframe>
                                  @else
                                    <img src="{{ route('rkap-files.view', $file['id']) }}"
                                      class="img-fluid p-3 rkap-preview-image" />
                                  @endif
                                </div>
                                <div class="modal-footer">
                                  <a href="{{ route('rkap-files.download', $file['id']) }}"
                                    class="btn btn-primary btn-sm"><i class="bx bx-download me-1"></i> Download</a>
              ...
              <div class="col-md-6 d-flex flex-column align-items-end">
                ...
                <span class="text-muted small fw-bold text-primary fs-6 has-tooltip">
                  ...
                  <span class="custom-tooltip-content tooltip-align-right">
                    ...
                    @else
                      <div class="text-center text-white-50 py-1">{{ __('Tidak ada data di periode sebelumnya') }}
                      </div>
                    @endif
                  </span>
                </span>
            ...
            <div class="row g-3 mb-3">
              ...
              <div class="col-md-3">
                ...
                <input type="text" class="form-control form-control-sm"
                  ...
                  list="satuan-options" placeholder="{{ __('Paket, Unit, ...') }}" autocomplete="off"
                  @disabled($isApproved)>
              </div>
            </div>
            ...
            @if (!empty($wp['work_plan_id']) && !empty($act['activity_id']))
              @if (!$hasMappedCoas)
                <div class="alert alert-danger d-flex align-items-center p-3 mb-3 animate__animated animate__fadeIn"
                  role="alert">
                  <i class="bx bx-error-circle me-3 fs-4"></i>
                  <div>
                    <h6 class="alert-heading mb-1 fw-bold text-danger">{{ __('Kegiatan Belum Terpetakan ke COA') }}
                    </h6>
                  ...
                  <table class="table table-sm table-bordered align-middle mb-2">
                    <thead class="table-primary text-white fw-semibold">
                      <tr>
                        ...
                        <th class="text-center align-middle rkap-w-10p">{{ __('Vol') }} <span
                    ...
                    <tbody>
                      ...
                      @foreach ($groups as $gIdx => $group)
                        @php
                          ...
                          $isCoaLockedForKepalaBiro =
                              ($isKepalaBiroUser ?? false) && (bool) ($firstBi['is_coa_locked'] ?? false);
                        @endphp

                        <tr
                          wire:key="wp-{{ $wp['_uid'] }}-act-{{ $actUid }}-group-{{ $gIdx }}-coa"
                          ...
                          <td colspan="6"
                            ...
                            @php
                              $groupSubtotal = collect($group['items'])->sum(function ($info) {
                                  ...
                                  $qty2 = !empty($bi['unit_2']) ? (float) ($bi['quantity_2'] ?? 1) : 1;
                                  $itemTotal =
                                      ((float) ($bi['quantity'] ?? 0)) * $qty2 * ((float) ($bi['unit_price'] ?? 0));
                            ...
                            <div class="position-relative">
                              <div class="input-group input-group-sm">
                                <input type="text"
                                  ...
                                  @input="if (!@js($isCoaLockedForKepalaBiro) && @js($hasMappedCoas)) { open = true; $dispatch('coa-dropdown-open') }"
                                  autocomplete="off" @disabled($isApproved || $isCoaLockedForKepalaBiro || !$hasMappedCoas)>
                                ...
                                @endphp
                                <span
                                  class="input-group-text px-2 fw-semibold text-nowrap has-tooltip rkap-coa-sum-chip">
                                  ...
                                  <span class="custom-tooltip-content tooltip-align-right">
                                    @if ($prevCoaData)
                                      ...
                                      <div class="row text-center">
                                        <div class="col-4 border-end">
                                          <div class="text-white-50 small rkap-tooltip-stat-label">
                                            {{ __('Anggaran') }}</div>
                                          <div class="fw-bold text-white rkap-tooltip-stat-value">Rp
                              ...
                              <select
                                ...
                                @if ($firstBi['coa_id'])
                                  ...
                                  @if ($selectedCoaForSelect)
                                    <option value="{{ $selectedCoaForSelect->id }}" selected>
                                      {{ $selectedCoaForSelect->code }} — {{ $selectedCoaForSelect->title }}
                                    </option>
                                  @endif
                                @endif
                              </select>
                              <div x-show="open && !@js($isCoaLockedForKepalaBiro)" x-cloak wire:ignore.self
                                ...
                                <template x-for="coa in filteredCoas" :key="coa.id">
                                  <div class="px-3 py-2 cursor-pointer dropdown-item small"
                                    ...
                                    <span class="fw-semibold text-primary"
                                      :class="selectedId == coa.id ? 'text-white' : ''" x-text="coa.code"></span>
                                ...
                                <div class="px-3 py-2 text-muted small border-top bg-light">
                                  <i
                                    class="bx bx-info-circle me-1"></i>{{ __('Ketik untuk mencari dari seluruh master COA.') }}
                                </div>
                              </div>
                            </div>
                          </td>
                        ...
                        @foreach ($group['items'] as $itemIdx => $itemInfo)
                          ...
                          <tr
                            ...
                            <td class="text-end text-nowrap border-top-0">
                              <div class="fw-semibold text-primary">Rp {{ number_format($biTotal, 0, ',', '.') }}
                              </div>
                            </td>
                ...
                @foreach ($act['budget_items'] as $biIdx => $bi)
                  ...
                  <div wire:key="modal-{{ $wp['_uid'] }}-{{ $actUid }}-{{ $biIdx }}"
                    ...
                    <div x-show="modalKey === '{{ $modalKey }}'"
                      ...
                      class="bg-white rounded-3 shadow-lg rkap-modal-shell" @click.stop>
                      {{-- Modal Header --}}
                      <div
                      ...
                      <div class="px-4 py-3 rkap-modal-body-scroll">
                          ...
                          @if (\App\Models\Coa::isForexAccount($bi['account_code'] ?? ''))
                            <div class="mb-3 d-flex align-items-center gap-2 mt-2">
                              ...
                              <div class="btn-group btn-group-sm" role="group">
                                <input type="radio" class="btn-check" name="gain-{{ $modalKey }}"
                                  id="gain-yes-{{ $modalKey }}" value="1"
                                  wire:model="workPlans.{{ $wpIdx }}.activities.{{ $actIdx }}.budget_items.{{ $biIdx }}.is_gain"
                                  @disabled($isApproved)>
                                <label class="btn btn-outline-success px-3 d-flex align-items-center"
                                  for="gain-yes-{{ $modalKey }}"><i
                                    class="bx bx-trending-up me-1"></i>Gain</label>

                                <input type="radio" class="btn-check" name="gain-{{ $modalKey }}"
                                  id="gain-no-{{ $modalKey }}" value="0"
                                  wire:model="workPlans.{{ $wpIdx }}.activities.{{ $actIdx }}.budget_items.{{ $biIdx }}.is_gain"
                                  @disabled($isApproved)>
                                <label class="btn btn-outline-danger px-3 d-flex align-items-center"
                                  for="gain-no-{{ $modalKey }}"><i
                                    class="bx bx-trending-down me-1"></i>Loss</label>
                              </div>
                            </div>
                          @endif
                          <div class="d-flex align-items-center mb-1">
                            <label
                              class="form-label small text-muted mb-0">{{ __('Pilih Bulan Distribusi Penganggaran:') }}</label>
                          </div>
                        ...
                        <div class="mb-4">
                          ...
                          @if (($bi['cashflow_group_code'] ?? '') === 'CF0B9')
                            <div class="mb-3 d-flex align-items-center gap-2 mt-2">
                              ...
                              <div class="btn-group btn-group-sm" role="group">
                                <input type="radio" class="btn-check" name="flow-{{ $modalKey }}"
                                  id="flow-in-{{ $modalKey }}" value="IN"
                                  wire:model="workPlans.{{ $wpIdx }}.activities.{{ $actIdx }}.budget_items.{{ $biIdx }}.flow_direction"
                                  @disabled($isApproved)>
                                <label class="btn btn-outline-success px-3 d-flex align-items-center"
                                  for="flow-in-{{ $modalKey }}"><i class="bx bx-trending-up me-1"></i>IN (Uang
                                  Masuk)</label>

                                <input type="radio" class="btn-check" name="flow-{{ $modalKey }}"
                                  id="flow-out-{{ $modalKey }}" value="OUT"
                                  wire:model="workPlans.{{ $wpIdx }}.activities.{{ $actIdx }}.budget_items.{{ $biIdx }}.flow_direction"
                                  @disabled($isApproved)>
                                <label class="btn btn-outline-danger px-3 d-flex align-items-center"
                                  for="flow-out-{{ $modalKey }}"><i class="bx bx-trending-down me-1"></i>OUT
                                  (Uang Keluar)</label>
                              </div>
                            </div>
                          @endif
                          <div class="d-flex align-items-center mb-1">
                            <label
                              class="form-label small text-muted mb-0">{{ __('Pilih Bulan Pendanaan:') }}</label>
                          </div>
                          <div class="d-flex flex-wrap gap-1 mb-2">
                            @foreach ($monthLabels as $monthNum => $monthLabel)
                              <button type="button"
                            ...
                            <table class="table table-sm table-bordered mb-0 rkap-summary-table">
                              <thead class="table-primary">
                                <tr>
                                  ...
                                  <th class="text-end">
                                    ...
                                    <div class="small fw-normal text-muted rkap-summary-note">
                                      ({{ $bi['coa_group_name'] ?: '-' }})</div>
                                  </th>
                                  <th class="text-end">
                                    Rencana Pendanaan (Rp)
                                    <div class="small fw-normal text-muted rkap-summary-note">
                                      ({{ $bi['cashflow_group_name'] ?: '-' }})</div>
                                  </th>
                                  <th class="text-end">
                                    Selisih (Rp)
                                    @php
                                      $coaModel = !empty($bi['coa_id'])
                                          ? \App\Models\Coa::with('differenceGroups')->find($bi['coa_id'])
                                          : null;
                                      $mappedDgs = $coaModel ? $coaModel->differenceGroups : collect();
                                    @endphp
                                    @if ($mappedDgs->count() > 1)
                                      @php
                                        $usedDgIds = $this->getUsedDifferenceGroupIds(
                                            (int) $wpIdx,
                                            (int) $actIdx,
                                            (int) $biIdx,
                                        );
                                      ...
                                      <div>
                                        <select
                                          class="form-select form-select-sm d-inline-block w-auto mt-1 rkap-font-07 py-0 px-2"
                                          wire:model.live="workPlans.{{ $wpIdx }}.activities.{{ $actIdx }}.budget_items.{{ $biIdx }}.difference_group_id">
                              ...
                              <tbody>
                                @foreach ($monthLabels as $monthNum => $monthLabel)
                                  ...
                                  @if ($isDistribMonth || $isCashOutMonth)
                                    @php
                                      $distValue = $isDistribMonth
                                          ? (float) ($bi['monthly_distribution'][$monthNum] ?? 0)
                                          : 0;
                                      $cashOutValue = $isCashOutMonth
                                          ? (float) ($bi['cash_out_distribution'][$monthNum] ?? 0)
                                          : 0;
                                      ...
                                      <td class="text-end align-middle">
                                        @if ($isDistribMonth)
                                          <div x-data="{
                                            ...
                                            <span class="input-group-text rkap-rp-chip">Rp</span>
                                            <input type="text"
                                              class="form-control form-control-sm text-end rkap-rp-input"
                              ...
                              <tfoot class="table-light">
                                ...
                                <tr>
                                  ...
                                  <th
                                    class="text-end small {{ $totalSelisih < 0 ? 'text-danger' : ($totalSelisih > 0 ? 'text-success' : 'text-muted') }}">
                                ...
                                <tr>
                                  ...
                                  <th
                                    class="text-end small {{ $sisaSelisih < 0 ? 'text-danger' : ($sisaSelisih > 0 ? 'text-success' : 'text-muted') }}">
                        ...
                        @else
                          <div class="text-center text-muted small py-3 border rounded-2">
                            <i
                              class="bx bx-info-circle me-1"></i>{{ __('Belum ada bulan yang dipilih. Pilih bulan di atas untuk') }}
    ...
    <div class="card-body d-flex justify-content-between align-items-center">
      ...
      <div class="d-flex gap-2">
        <button wire:click="saveDraft()" @click="isSubmitting = true" wire:loading.attr="disabled"
          @disabled($this->isSubmissionClosed) class="btn btn-label-primary">
          <span wire:loading.remove wire:target="saveDraft"><i class="bx bx-save me-1"></i>
            {{ __('Simpan Draft') }}</span>
        ...
        <button wire:click="submitForReview()" @click="isSubmitting = true" wire:loading.attr="disabled"
          ...
          wire:confirm="{{ __('Yakin mengajukan RKAP ini untuk review? Pastikan data sudah lengkap.') }}"
          class="btn btn-primary">
          <span wire:loading.remove wire:target="submitForReview"><i class="bx bx-send me-1"></i>
            {{ __('Ajukan untuk') }}
            {{ __('Review') }}</span>
  ...
  <div class="modal fade" id="uploadFileModal" tabindex="-1" aria-hidden="true" wire:ignore.self>
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title"><i class="bx bx-upload me-2 text-primary"></i>{{ __('Upload File Referensi') }}
          </h5>
        ...
        <div class="modal-body text-start">
          <form wire:submit.prevent="handleFileUpload">
            <div class="mb-3">
              ...
              <input type="file" class="form-control @error('referenceFile') is-invalid @enderror"
                wire:model="referenceFile">
              @error('referenceFile')
                <div class="invalid-feedback d-block">{{ $message }}</div>
              @enderror
              <div class="form-text text-muted small mt-1">
                {{ __('Tipe file yang diperbolehkan: Word (doc, docx), Excel (xls, xlsx), PDF, Zip, dan Gambar. Maksimal ukuran 5 MB.') }}
              </div>
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-primary" wire:click="handleFileUpload" wire:loading.attr="disabled"
            wire:target="referenceFile" @disabled(!$referenceFile)>
            Upload
          </button>
        </div>
      </div>
